...
向前Euler格式 (GRP/Gdounov\_solver\_source.c):
\begin{align*}
...
\begin{itemize}
\item 拉式Gdounov格式 (Gdounov\_solver\_source.c)
$$
\left(\frac{D u}{D t}\right)_{i+1/2}^{n}=0, \quad \left(\frac{D p}{D t}\right)_{i+1/2}^{n}=0
$$. 

\item 拉式GRP格式 (GRP\_solver\_source.c)

通过斜率限制器重构得到的斜率求出接触间断处的物质导数, 由GRP解法器 (linear\_GRP\_solver\_LAG.c) 计算出时间导数, 通过完全显式向前Euler格式更新, 再计算出斜率.
\end{itemize}
