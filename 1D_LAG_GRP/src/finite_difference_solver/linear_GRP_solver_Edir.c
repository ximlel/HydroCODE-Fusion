 ...
 double gamma, double eps)
{
  ...
  double c_L, c_R;
  int CRW[2];
  double rho_star, u_star, p_star, c_star, rho_star_L, rho_star_R, c_star_L, c_star_R;
  ...
  dist = (u_L-u_R)*(u_L-u_R) + (p_L-p_R)*(p_L-p_R) + (rho_L-rho_R)*(rho_L-rho_R);
  dist = sqrt(dist);
//=========acoustic case==========
  if(dist < eps)
  {
    ...
    else
    {
      ...
      c_star = sqrt(gamma * p_star / rho_star);

	mid[0] = rho_star;
	mid[1] =   u_star;
	mid[2] =   p_star;
      
	direvative[1] = -0.5*((u_star+c_star)*(s_u_L+s_p_L/rho_star/c_star)+(u_star-c_star)*(s_u_R-s_p_R/rho_star/c_star));
	direvative[2] = -0.5*rho_star*c_star*((u_star+c_star)*(s_u_L+s_p_L/rho_star/c_star)-(u_star-c_star)*(s_u_R-s_p_R/rho_star/c_star));

      if(u_star > 0.0)
      {
	direvative[0] = (u_star*(s_p_L - s_rho_L*c_star*c_star) + direvative[2])/c_star/c_star;
      }
      else
      {
	direvative[0] = (u_star*(s_p_R - s_rho_R*c_star*c_star) + direvative[2])/c_star/c_star;
      }
...
//----------solving the LINEAR GRP----------
  ...
  else
  {
    if((CRW[0]) && ((u_star-c_star_L) > 0.0)) // the t-axe is in a 1-CRW
    {
      ...
      mid[2] = p_L*pow(mid[1]/c_L,2*gamma/(gamma-1.0));
    ...
    else if((CRW[1]) && ((u_star+c_star_R) < 0.0)) // the t-axe is in a 3-CRW
    {
      ...
      mid[2] = p_R*pow(mid[1]/c_R,2*gamma/(gamma-1.0));
      mid[0] = gamma*mid[2]/mid[1]/mid[1];
