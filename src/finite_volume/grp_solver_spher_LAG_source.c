...
void grp_solver_spher_LAG_source(struct flu_var *FV, struct spher_mesh_var *smv, const int M, double * cpu_time, const int N_plot , double time_plot[])
{
    ...
    double *CC  = (double*)ALLOC(Md*sizeof(double));
    ...
    Smax_deltar=0.;
    Smax_deltar=(Smax_deltar>(fabs(ifv_L.U)+CL)/dr ? Smax_deltar : (fabs(ifv_L.U)+CL)/dr);
    Smax_deltar=(Smax_deltar>(fabs(ifv_R.U)+CR)/dr ? Smax_deltar : (fabs(ifv_R.U)+CR)/dr);
    dt=CFL/Smax_deltar;
    ...
    */
    ...
    for(k=1; k<=N; k++)
	{
	    ...
	    for(i=0; i<Ncell; i++)
		{
		    ...
		    GRPsolverSLag(wave_speed, dire, mid, &ifv_L, &ifv_R, r, M);

		    Smax_deltar=Smax_deltar>fabs(wave_speed[0])/Ddr[i] ? Smax_deltar:fabs(wave_speed[0])/Ddr[i];
		    Smax_deltar=Smax_deltar>fabs(wave_speed[1])/Ddr[i+1]?Smax_deltar:fabs(wave_speed[1])/Ddr[i+1];
	    ...
	    for(i=0;i<Ncell;i++)
		{
		    ...
		    UL          = UU[i]+(0.5*(Rb[i]+Rb[i+1])-RR[i])*ifv_L.d_u;
		    VL          = 0.5*(Rb[i]+Rb[i+1])*tan(0.5*dtheta)*TVL;
		    ifv_R   =  ifv_L;
		    ifv_L.U = -UL*sin(0.5*dtheta)+VL*cos(0.5*dtheta);
		    ifv_R.U = -ifv_L.U;
					
		    StarPU(mid, &ifv_L, &ifv_L);
		    r=0.5*(Rb[i]+Rb[i+1])/cos(0.5*dtheta);
		    ifv_L.RHO = mid[0];
		    AcousticSLagTangent(dire, &ifv_L, r, M);
		    F2P[i]=PM+dt*DtP;
		...
		{
		    file_spher_write_TEC(time_plot[N_count])；
			N_count++;
		}
