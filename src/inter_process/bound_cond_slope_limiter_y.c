				 ...
				 struct b_f_var * bfv_D, struct b_f_var * bfv_U, _Bool find_bound_y)
{
    ...
    for(j = 0; j < m; ++j)
	{
	    minmod_limiter(false, n, find_bound_y, CV->s_u[j],   CV[nt].U[j],   bfv_D[j].U,   bfv_U[j].U,   h_y);
	    minmod_limiter(false, n, find_bound_y, CV->s_v[j],   CV[nt].V[j],   bfv_D[j].V,   bfv_U[j].V,   h_y);
	    minmod_limiter(false, n, find_bound_y, CV->s_p[j],   CV[nt].P[j],   bfv_D[j].P,   bfv_U[j].P,   h_y);
	    minmod_limiter(false, n, find_bound_y, CV->s_rho[j], CV[nt].RHO[j], bfv_D[j].RHO, bfv_U[j].RHO, h_y);
	}
