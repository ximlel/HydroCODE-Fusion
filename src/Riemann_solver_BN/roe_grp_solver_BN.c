...
/*
  c_g_S = sqrt(gamma_g*p_g_S*U[0]/U[1]);
  c_l_S = sqrt(gamma_l*p_l_S*(1.0-U[0])/U[4]);
  H_g_S = gamma_g/(gamma_g-1.0)*p_g_L*U[0]/U[1] + 0.5*(u_g_S*u_g_S);
  H_l_S = gamma_l/(gamma_l-1.0)*p_l_L*(1.0-U[0])/U[4] + 0.5*(u_l_S*u_l_S);
  b1 =  gamma_g*P_INT/(c_g_S*c_g_S-(u_g_S-U_INT)*(u_g_S-U_INT));
  b2 = -gamma_l*P_INT/(c_l_S*c_l_S-(u_l_S-U_INT)*(u_l_S-U_INT));
  R_inv[0][0] = 1.0;
  R_inv[0][1] = 0.0;
  ...
  R_inv[1][6] = 0.0;
  R_inv[2][0] = U_INT*b1;
  R_inv[2][1] = u_g_S - c_g_S;
  R_inv[2][2] = u_g_S;
  R_inv[2][3] = u_g_S + c_g_S;
  R_inv[2][4] = 0.0;
  R_inv[2][5] = 0.0;
  R_inv[2][6] = 0.0;
  R_inv[3][0] = (H_g_S-u_g_S*(u_g_S-U_INT))*b1-P_INT;
  R_inv[3][1] = H_g_S - u_g_S*c_g_S;
  R_inv[3][2] = 0.5*(u_g_S*u_g_S);
  R_inv[3][3] = H_g_S + u_g_S*c_g_S;
  R_inv[3][4] = 0.0;
  R_inv[3][5] = 0.0;
  R_inv[3][6] = 0.0;
  R_inv[4][0] = b2;
  ...
  R_inv[5][2] = 0.0;
  R_inv[5][3] = 0.0;
  R_inv[5][4] = u_l_S - c_l_S;
  R_inv[5][5] = u_l_S;
  R_inv[5][6] = u_l_S + c_l_S;
  R_inv[6][0] = (H_l_S-u_l_S*(u_l_S-U_INT))*b2+P_INT;
  R_inv[6][1] = 0.0;
  R_inv[6][2] = 0.0;
  R_inv[6][3] = 0.0;
  R_inv[6][4] = H_l_S - u_l_S*c_l_S;
  R_inv[6][5] = 0.5*(u_l_S*u_l_S);
  R_inv[6][6] = H_l_S + u_l_S*c_l_S;
*/
 ...
 double d_v_g_S, d_v_l_S, t_v_g_S, t_v_l_S;
  ...
  t_p_l_S =  ((T_U_S[6] - 0.5*(T_U_S[5]*u_l_S + U[5]*t_u_l_S))*(gamma_l-1.0) + p_l_S*T_U_S[0])/(1.0-U[0]);
   ...
   Dt_U_all[0] = -U_INT*D_U_S[0]-V_INT*T_U_S[0];
   Dt_U_all[1] = -D_U_S[2]-T_U_S[2];
   Dt_U_all[2] = -D_U_S[2]*u_g_S-U[2]*d_u_g_S-U[0]*d_p_g_S+(P_INT-p_g_S)*D_U_S[0]-T_U_S[2]*v_g_S-U[2]*t_v_g_S;
   Dt_U_all[3] = -D_U_S[2]*v_g_S-U[2]*d_v_g_S-T_U_S[1]*v_g_S*v_g_S-U[1]*2.0*v_g_S*t_v_g_S-U[0]*t_p_g_S+(P_INT-p_g_S)*T_U_S[0];
   Dt_U_all[4] = -D_U_S[3]*u_g_S-U[3]*d_u_g_S-U[0]*(u_g_S*d_p_g_S+d_u_g_S*p_g_S)+(P_INT*U_INT-p_g_S*u_g_S)*D_U_S[0]-T_U_S[3]*v_g_S-U[3]*t_v_g_S-U[0]*(v_g_S*t_p_g_S+t_v_g_S*p_g_S)+(P_INT*V_INT-p_g_S*v_g_S)*T_U_S[0];
   Dt_U_all[5] = -D_U_S[5]-T_U_S[5];;
   Dt_U_all[6] = -D_U_S[5]*u_l_S-U[5]*d_u_l_S-(1.0-U[0])*d_p_l_S-(P_INT-p_l_S)*D_U_S[0]-T_U_S[5]*v_l_S-U[5]*t_v_l_S;
   Dt_U_all[7] = -D_U_S[5]*v_l_S-U[5]*d_v_l_S-T_U_S[4]*v_g_S*v_g_S-U[4]*2.0*v_l_S*t_v_l_S-(1.0-U[0])*t_p_l_S-(P_INT-p_l_S)*T_U_S[0];
   Dt_U_all[8] = -D_U_S[6]*u_l_S-U[6]*d_u_l_S+(1.0-U[0])*(u_l_S*d_p_l_S+d_u_l_S*p_l_S)-(P_INT*U_INT-p_l_S*u_l_S)*D_U_S[0]-T_U_S[6]*v_l_S-U[6]*t_v_l_S-(1.0-U[0])*(v_l_S*t_p_l_S+t_v_l_S*p_l_S)-(P_INT*V_INT-p_l_S*v_l_S)*T_U_S[0];
